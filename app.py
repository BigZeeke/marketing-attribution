...
def warn(msg):   st.markdown(
    f'<div class="insight-warn">⚠️ {msg}</div>', unsafe_allow_html=True)


def danger(msg): st.markdown(
    f'<div class="insight-danger">🔴 {msg}</div>', unsafe_allow_html=True)


def good(msg):   st.markdown(
    f'<div class="insight-good">✅ {msg}</div>', unsafe_allow_html=True)
...
{{"recommendations":[{{"priority":"High","title":"Short title","problem":"1-2 sentences","action":"2-3 sentences","impact":"estimated impact","effort":"Low/Medium/High","owner":"Media/CRM/Product/Leadership"}}]}}"""
    ...
    with st.spinner("Calling Claude API..."):
        try:
            ...
            raw = result["content"][0]["text"].strip().replace(
                "```json", "").replace("```", "").strip()
...
if st.session_state.get("show_ai"):
    ...
    with col_a:
        if st.session_state.get("ai_recs"):
            st.success(
                f"✓ {len(st.session_state.ai_recs)} recommendations generated from live data")
    with col_b:
    ...
    if page == PAGES[0]:
        ...
        st.caption(
            "Comparing First Touch · Last Touch · Linear · Time Decay · U-Shaped")
        ...
        c1.metric("Total Attributed Revenue",
                  f"${model_df['linear'].sum():,.0f}")
        ...
        if model_choice == "All Models Side by Side":
            melt = model_df.melt(id_vars=["channel_name"],
                                 value_vars=["first_touch", "last_touch",
                                             "linear", "time_decay", "u_shaped"],
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(name="First Touch",
                          x=model_df["channel_name"], y=model_df["first_touch"]))
            fig.add_trace(go.Bar(name="Last Touch",
                          x=model_df["channel_name"], y=model_df["last_touch"]))
            fig.update_layout(
                barmode="group", title="First Touch vs Last Touch Attribution")
            st.plotly_chart(fig, use_container_width=True)
            ...
            fig = go.Figure()
            fig.add_trace(
                go.Bar(name="Linear",  x=model_df["channel_name"], y=model_df["linear"]))
            fig.add_trace(
                go.Bar(name="U-Shaped", x=model_df["channel_name"], y=model_df["u_shaped"]))
            fig.update_layout(
                barmode="group", title="Linear vs U-Shaped Attribution")
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    # ══════════════════════════════════════════════════════════════════════════════
        ...
        st.caption(
            "Source: Google Ads · Meta Ads — daily spend vs. attributed revenue (Linear model)")
        ...
        roi_df = spend_df.merge(
            rev_df, on="channel_name", how="left").fillna(0)
        roi_df["roas"] = (roi_df["attributed_rev"] /
                          roi_df["total_spend"].replace(0, 1)).round(2)

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Total Paid Spend", f"${roi_df['total_spend'].sum():,.0f}")
        c2.metric("Total Attributed Rev",
                  f"${roi_df['attributed_rev'].sum():,.0f}")
        below_break = roi_df[roi_df["roas"] < 1.0]
        c3.metric("Channels Below Break-Even", f"{len(below_break)}",
                  delta=f"-{len(below_break)} channels losing money", delta_color="inverse")
    ...
    # ══════════════════════════════════════════════════════════════════════════════
        ...
        st.caption(
            "Awareness → Engagement → Conversion · Source: GA4 · HubSpot · Segment")
        ...
        r = funnel_df.iloc[0]
        engaged_rate = round(r["engaged"]/r["prospects"]
                             * 100, 1) if r["prospects"] else 0
        conv_rate = round(r["converted"]/r["engaged"] *
                          100, 1) if r["engaged"] else 0
        ...
        if conv_rate < 20:
            danger(
                f"Engaged-to-converted rate is <b>{conv_rate}%</b>. Over 80% of engaged prospects are dropping before converting — examine landing page experience, offer strength, and friction in the signup flow.")

        fig = go.Figure(go.Funnel(
            y=["Prospects Reached", "Engaged", "Converted",
                "Trial Start", "Paid Subscriber"],
            x=[r["prospects"], r["engaged"], r["converted"],
                r["trials"], r["subscribers"]],
            textinfo="value+percent initial",
            marker_color=["#1F4E79", "#2E75B6",
                          "#4BACC6", "#70AD47", "#A9D18E"]
        ))
        ...
        fig.add_hline(y=avg_cvr, line_dash="dash",
                      annotation_text=f"Avg {avg_cvr:.1f}%")
        st.plotly_chart(fig, use_container_width=True)
    ...
    # ══════════════════════════════════════════════════════════════════════════════
        ...
        st.caption(
            "Source: HubSpot · Mixpanel — 30/60/90/180/365-day cohort windows")
        ...
        if not ltv_df.empty:
            ...
            best = ltv_df.iloc[0]
            worst = ltv_df.iloc[-1]
            c1.metric("Best 365-Day LTV Channel",
                      best["acquisition_channel"], f"${best['ltv_365']:,.2f}")
            c2.metric("Worst 365-Day LTV Channel",
                      worst["acquisition_channel"], f"${worst['ltv_365']:,.2f}")
            c3.metric("LTV Gap (Best vs Worst)",
                      f"${best['ltv_365']-worst['ltv_365']:,.2f}")
            ...
            for _, row in ltv_df.iterrows():
                if row["ltv_90"] > 0 and (row["ltv_365"]-row["ltv_60"])/row["ltv_60"] < 0.15:
                    danger(
                        f"<b>{row['acquisition_channel']}</b> LTV flatlines after 60 days — customers acquired here are churning early. Check whether acquisition targeting is pulling low-intent users.")
            ...
            if not occ_row.empty:
                ...
                if occ["ltv_365"] < occ["ltv_60"]*1.2:
                    danger(
                        f"<b>Occasional segment</b> LTV flatlines after 60 days (60-day: ${occ['ltv_60']:,.2f} vs 365-day: ${occ['ltv_365']:,.2f}). These customers are not converting to habit — consider an engagement campaign at day 45-50 before churn occurs.")

        ltv_melt = ltv_df.melt(id_vars=["acquisition_channel"],
                               value_vars=["ltv_30", "ltv_60",
                                           "ltv_90", "ltv_180", "ltv_365"],
                               var_name="Window", value_name="LTV")
        ltv_melt["Window"] = ltv_melt["Window"].str.replace(
            "ltv_", "").apply(lambda x: f"{x}-Day")
    ...
    # ══════════════════════════════════════════════════════════════════════════════
        ...
        st.caption(
            "Source: Klaviyo · Mixpanel — abandoned cart flows, win-back campaigns")
        ...
        abandons = int(cart_df[cart_df["event_type"] == "Abandon"]
                       ["events"].sum()) if len(cart_df) else 0
        recovers = int(cart_df[cart_df["event_type"] == "Recover"]
                       ["events"].sum()) if len(cart_df) else 0
        ...
        avg_val = float(cart_df[cart_df["event_type"] == "Abandon"]
                        ["avg_value"].mean()) if len(cart_df) else 0
        ...
        if not churned_react.empty:
            ...
            if avg_churned_cvr < 8:
                danger(
                    f"<b>Win-back campaigns targeting Churned customers</b> are converting at only {avg_churned_cvr:.1f}%. This spend is largely wasted. Recommend pausing win-back for Churned and reallocating budget to At-Risk campaigns which convert at significantly higher rates.")
        if not atrisk_react.empty:
            avg_atrisk_cvr = atrisk_react["cvr"].mean()
            good(
                f"<b>At-Risk reactivation campaigns</b> convert at {avg_atrisk_cvr:.1f}% — intervening before churn is significantly more effective than win-back. Recommend earlier trigger logic (day 21 inactivity vs current thresholds).")

        c1, c2, c3 = st.columns(3)
        c1.metric("Campaigns Sent", f"{react_df['sent'].sum():,}")
        c2.metric("Customers Won Back", f"{react_df['conversions'].sum():,}")
        c3.metric("Revenue Won Back",
                  f"${react_df['revenue_won_back'].sum():,.0f}")
    ...
    # ══════════════════════════════════════════════════════════════════════════════
        ...
        st.caption(
            "Non-paid analytics: organic reach, sentiment, NPS, branded search · Source: GA4 · HubSpot")
        ...
        if not brand_df.empty:
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Avg Share of Voice",
                      f"{brand_df['share_of_voice'].mean()*100:.1f}%")
            c2.metric("Avg NPS Score", f"{brand_df['nps_score'].mean():.1f}")
            c3.metric("Avg Sentiment",
                      f"{brand_df['sentiment_score'].mean():.2f}")
            c4.metric("Total Branded Searches",
                      f"{brand_df['branded_searches'].sum():,}")

            nps_avg = brand_df["nps_score"].mean()
            if nps_avg < 40:
                danger(
                    f"NPS averaging <b>{nps_avg:.1f}</b> — below the 'Good' threshold of 50. Customer satisfaction issues will compound churn. Recommend NPS deep-dive to identify detractor themes.")
            elif nps_avg > 60:
                good(
                    f"NPS averaging <b>{nps_avg:.1f}</b> — strong promoter base. Consider activating referral program to convert brand satisfaction into acquisition channel.")
    ...
    # ══════════════════════════════════════════════════════════════════════════════
        ...
        st.caption(
            "Simulated platform data — no real API connections · 14 tables · 95,000+ rows")
